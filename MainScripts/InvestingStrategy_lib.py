...
def load_or_update_ticker_mapping(indices_dict, mapping_file="Ticker_Mapping_Cache.json"):
    """
...
def run_historical_backtest(indices_dict, months_to_simulate=12, monthly_contribution=1000.0, 
    ...
    if export_excel: 
        output_filename = f"BackTests/Backtest_Results_{today.strftime('%Y%m%d_%H%M%S')}.xlsx"
        candidates_filename = f"BackTests/TopCandidates_Results_{today.strftime('%Y%m%d_%H%M%S')}.xlsx"
